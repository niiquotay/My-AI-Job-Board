  ...
  const handleDownloadPDF = (targetJob?: Job) => {
    try {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text("© CaliberDesk Enterprise Protocol • CONFIDENTIAL", 105, 285, { align: 'center' });
      }
          ...
          <div className="pt-3 border-t border-white/5 flex flex-col md:flex-row gap-4">
            <div className="flex-1 space-y-1">
              ...
              <p className="text-[9px] text-white/60 leading-relaxed font-medium">
                "CaliberDesk Neural Engine increased by <span className="text-[#41d599] font-black">12%</span>. Correlates with 'Culture-Fit' refinement."
              </p>
            </div>
              ...
              {jobs.map(job => {
                ...
                return (
                  <tr key={job.id} className="hover:bg-white/[0.02] group transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`px-2 py-0.5 rounded-md text-[7px] font-black uppercase tracking-widest border ${perfStatus ? 'bg-[#F0C927]/10 text-[#F0C927] border-[#F0C927]/20' : 'bg-white/5 text-white/20 border-white/10'
                        }`}>
                        {perfStatus || "Standby"}
                      </span>
                    </td>
