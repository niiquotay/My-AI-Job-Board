   ...
   const handleCVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
         ...
         reader.onload = async () => {
            ...
            const filename = `cv-${user.id || 'guest'}-${Date.now()}.${file.name.split('.').pop()}`;
            const cvUrl = await uploadFile('cvs', filename, file);
            ...
            setToast({ message: "Neural sync complete. Manifest and CV persisted.", type: 'success' });
         };
         reader.readAsDataURL(file);
      } catch (err) {
         console.error('CV Sync Error:', err);
         setToast({ message: "CV parsing or persistence fault.", type: 'error' });
      } finally {
         setIsParsing(false);
      }
   };
      ...
      try {
         if (user.email) {
            ...
            const { error } = await supabase.from('profiles').upsert({
               ...user,
               full_name: user.name, // Maps to SQL
               updated_at: new Date().toISOString()
            });
            if (error) throw error;
            setToast({ message: "Identity manifest synchronized with cloud.", type: 'success' });
         } else {
            setToast({ message: "Changes cached locally. Sign in to persist to cloud.", type: 'info' });
         }
         setTimeout(() => onBack(), 1000);
      } catch (err: any) {
         console.error('Save Error:', err);
         setToast({ message: "Cloud synchronization fault.", type: 'error' });
      } finally {
         setIsSaving(false);
      }
   };
