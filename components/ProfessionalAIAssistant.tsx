          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 pr-3">
            {TEMPLATES.map((t) => (
              <button
                ...
                className={`w-full group p-2.5 rounded-[20px] border transition-all duration-300 text-left flex items-center gap-3 ${selectedTemplate.id === t.id
                    ? 'bg-[#F0C927]/10 border-[#F0C927]/40 shadow-[0_0_30px_rgba(240,201,39,0.08)] scale-[1.02]'
                    : 'bg-white/[0.02] border-white/5 hover:bg-white/5 hover:border-white/10'
                  }`}
              >
                <div className={`shrink-0 w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-500 ${selectedTemplate.id === t.id ? 'bg-[#F0C927] text-[#0a4179] rotate-[360deg]' : 'bg-white/5 text-white/30 group-hover:text-white'
                  }`}>
                  <t.icon size={16} />
                </div>
